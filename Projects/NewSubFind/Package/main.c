...
double rt_func(double M_c, double M_s, double r_t, double * Sat_Pos, double * Sat_Vel, double r){
	return G*M_in(r_t,M_s)/(pow(omega(Sat_Pos,Sat_Vel),2)+G*(2*M_in(r,M_c)*pow(r,-3)-4*pi*rho_nfw(r,M_c)))-pow(r_t,3);
}
...
double A_d(double M_c,double M_s,double r,double vt){//Use M_s or Ms0?? CAREFULL!!!!!!
	if (vt == 0) return 0;
...
double Tdf_NFW_C(double M_c, double M_s, double jmin,double epsilon,double alpha, double dt, double MAX, double dt_print, int verbose){
	int i = 0;
	double * Arr;
...
double * jM_eV_NFW_C(double M_c, double M_s, double jmin,double epsilon,double alpha, double dt, double MAX, double dt_print, int verbose){
	int i = 0;
	double * Arr;
	Arr = (double*)calloc((int) 3*(MAX/dt_print), sizeof(double));
	for (i=0; i < 3*(MAX/dt_print); i++) Arr[i] = -99;
