...
cdef extern from "Main_iso_Polar.h":
...
def Tdf(
    ...
    int verbose):
	dt_print = 0.1
	Val = Tdf_C( M_c,  M_s,  jmin, epsilon, alpha,  dt,  MAX, dt_print, verbose)
	return Val
...
def jM_eV(
    ...
    int verbose):
	dt_print = 0.1
	Val = jM_eV_C( M_c,  M_s,  jmin, epsilon, alpha,  dt,  MAX, dt_print, verbose)
	A = []
