...
double RK4_integrator(double * Arr, double M_c, double M_s, double * Sat_Pos, double * Sat_Vel, double jmin, double dt, double alpha, double MAX,  double dt_print, int verbose){
	...
	double r,j,Lim_rt,rp,Vtot,Msat,Ag,Ad,theta,gamma,vp_mod,t = 0;
	...
	while(j / j0 > jmin && M_s > 0){
		...
		if (i % (int) (dt_print/dt) == 0 && l < Narr){
			if (verbose) printf("%lf\t%lf\t%lf\t\t%G\t%G\t%G\t\t%G\t%G\t%G\t%G\t%G \n",t,r,j/j0,Sat_Pos[0],Sat_Pos[1],Sat_Pos[2],Sat_Vel[0],Sat_Vel[1],Sat_Vel[2],gamma,theta);
			Arr[l] = t;
			Arr[l+Narr] = M_s;
			Arr[l+2*Narr] = j;
			l++;
		...
		for (rk = 0; rk < 4; rk++){
			...
			theta = asin((SatP[0]*SatV[0]+SatP[1]*SatV[1]+SatP[2]*SatV[2])/rp/Vtot);
			gamma = asin(SatP[0]/rp);
			if (gamma < 0) gamma = pi/2.+gamma;
			Ad = Ad*cos(theta);
			
			Vp[0] = -cos(gamma)*SatP[1]/fabs(SatP[1]+2e-10)*j_vec[2]/fabs(j_vec[2]+1e-20);
			Vp[1] = sin(gamma)*SatP[0]/fabs(SatP[0]+2e-10)*j_vec[2]/fabs(j_vec[2]+1e-20);
			Vp[2] = 0;
			
			for (dim=0; dim < 3; dim++){
				k_dd[dim][rk] = -Ag*SatP[dim]/rp-Ad*Vp[dim];
				k_d[dim][rk]  = SatV[dim];
			}
...
// 		printf("%lf\t%lf\t%lf\t\t%G\t%G\t%G\t\t%G\t%G\t%G\t%G\t%G\t%G\t%G\t%G \n",t,r,j/j0,Sat_Pos[0],Sat_Pos[1],Sat_Pos[2],Sat_Vel[0],Sat_Vel[1],Sat_Vel[2],vp_mod,j_vec[2],gamma,Vp[0],Vp[1]);
		Cross_Product(Sat_Pos,Sat_Vel,j_vec);
